...
public class GameServer extends Thread{
        ...
        switch (type){
            ...
            case LOGIN:
                ...
                if(address.getHostAddress().equalsIgnoreCase("127.0.0.1")){
                    player=new MultiPlayer(1,1,game.key,game.level,((Packet00Login)packet).getUsername(),address,port);
                }else{
                    player=new MultiPlayer(1,1,game.key,game.level,((Packet00Login)packet).getUsername(),address,port);
                }
                if(player!=null){
                    this.conplayers.add(player);
                    game.level.players.add(player);
                }
    ...
    public void addConnection(MultiPlayer player, Packet00Login packet) {
        ...
        for(int i=0;i<conplayers.size();i++){
            if(player.getUsername().equalsIgnoreCase(conplayers.get(i).getUsername())){
                ...
                packet=new Packet00Login(conplayers.get(i).getUsername());
